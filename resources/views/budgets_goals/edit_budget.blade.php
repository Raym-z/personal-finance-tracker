        ...
        <div class="col-12 col-md-8 col-lg-6">
            <div class="card border-0 shadow-sm rounded-4">
                <div class="card-body p-4">
                    <h2 class="fw-bold mb-4" style="color: #2563eb;">Edit Budget</h2>
                    <form method="POST" action="{{ route('budgets.update', $budget->id) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <select id="category" name="category" class="form-select" required>
                                ...
                                @foreach($tags as $tag)
                                <option value="{{ $tag }}"
                                    {{ old('category', $budget->category) == $tag ? 'selected' : '' }}>{{ $tag }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" id="amount" name="amount" class="form-control" required min="0.01"
                                step="0.01" value="{{ old('amount', $budget->amount) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <select id="period" name="period" class="form-select" required>
                                <option value="monthly"
                                    {{ old('period', $budget->period) == 'monthly' ? 'selected' : '' }}>Monthly</option>
                                <option value="weekly"
                                    {{ old('period', $budget->period) == 'weekly' ? 'selected' : '' }}>Weekly</option>
                            </select>
